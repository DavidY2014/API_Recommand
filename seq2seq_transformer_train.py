...
def train(args):
    ...
    if args.visual:
        ...
        os.makedirs(f'./transformerOutput/{args.model}/{args.expname}/{timestamp}/models', exist_ok=True)
        os.makedirs(f'./transformerOutput/{args.model}/{args.expname}/{timestamp}/temp_results', exist_ok=True)
        fh = logging.FileHandler(f"./transformerOutput/{args.model}/{args.expname}/{timestamp}/logs.txt")
                                      # create file handler which logs even debug messages
        logger.addHandler(fh)# add the handlers to the logger
        tb_writer = SummaryWriter(f"./transformerOutput/{args.model}/{args.expname}/{timestamp}/logs/")
        # save arguments
        json.dump(vars(args), open(f'./transformerOutput/{args.model}/{args.expname}/{timestamp}/args.json', 'w'))
    ...
    def save_model(model, epoch, timestamp):
        ...
        os.makedirs(f'./transformerOutput/{args.model}/{args.expname}/{timestamp}/models', exist_ok=True)
        ckpt_path=f'./transformerOutput/{args.model}/{args.expname}/{timestamp}/models/model_epo{epoch}.pkl'
    ...
    itr_global=1
    itr_count = int(len(train_loader.dataset) / config['batch_size'])
    start_epoch=1
    for epoch in range(start_epoch, config['epochs']+1):
        epoch_start_time = time.time()
        itr_start_time = time.time()
        print(itr_start_time)
        #desc, desc_len, api, api_len
        for i, batch in enumerate(train_loader):
            model.train()
            batch_gpu = [tensor.to(device) for tensor in batch]
            loss = model.forward(*batch_gpu) 
            loss.backward()
            ...
            if itr_global % args.log_every == 0:
                ...
                log = '%s-%s|@gpu%d epo:[%d/%d] iter:[%d/%d] step_time:%ds loss:%f'\
                %(args.model, args.expname, args.gpu_id, epoch, config['epochs'],itr_global,itr_count, elapsed, loss)
                if args.visual:
            ...
            if itr_global % args.eval_every == 0:  # evaluate the model in the develop set
                ...
                os.makedirs(f'./transformerOutput/{args.model}/{args.expname}/{timestamp}/temp_results', exist_ok=True)
                f_eval = open(f"./transformerOutput/{args.model}/{args.expname}/{timestamp}/temp_results/iter{itr_global}.txt", "w")
                repeat = 1
                decode_mode = 'sample'
                recall_bleu, prec_bleu = evaluate(model, metrics, valid_loader, vocab_desc, vocab_api, repeat, decode_mode, f_eval)

                if args.visual:
                    tb_writer.add_scalar('transformer_recall_bleu', recall_bleu, itr_global)
                    tb_writer.add_scalar('transformer_prec_bleu', prec_bleu, itr_global)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='basic transformer')

    # Path Arguments
    parser.add_argument('--data_path', type=str, default='./data/', help='location of the data corpus')
    parser.add_argument('--model', type=str, default='Transformer_EncoderDecoder', help='model name: seq2seqTransformer')
